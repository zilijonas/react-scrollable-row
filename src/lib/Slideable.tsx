...
import { InfiniteSlide } from './InfiniteSlider';
import { SlideableProps } from './types';

const SlideableComponent: React.FC<SlideableProps> = props => {
  const {
    items,
    buttonsStyle,
    noButtons = false,
  ...
  return (
    <>
      ...
      >
        ...
        <div
          ref={useCallback((ref: HTMLDivElement) => setList(new ScrollableElement(ref, items.length)), [items.length])}
